  ...
  __eval(cmd, context, filename, callback) {
    ...
    co(function* () {
      ...
      if (cmd === 'exit' || cmd === 'quit') {
        ...
        if (self.context.__currentCursor instanceof Cursor && !self.context.__currentCursor.isClosed()) {
          const documents = [];
          ...
          let hasMore = documents.length === cutoff && !self.context.__currentCursor.isClosed();
      ...
      yield self.executor.executeAsync(finalCommand, self.context);
      ...
      if (self.context.__result instanceof Cursor && cmd !== 'it' /* && self.context.__result.cursor.s.state === 0 */) {
