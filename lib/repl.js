...
class REPL extends EventEmitter {
  ...
  __eval(cmd, _context, filename, callback) {
    let self = this;
    ...
    co(function* () {
      ...
      if (cmd === 'exit' || cmd === 'quit') {
        ...
        let result = yield self.state.client.db('admin').command({ listDatabases: true });
        ...
        let collectionNames = yield context.db.getCollectionNames();
        ...
        if (context.__currentCursor instanceof Cursor && !context.__currentCursor.isClosed()) {
          ...
          while (true) {
            let doc = yield context.__currentCursor.next();
            // No more entries
            if (!doc) break;
            documents.push(doc);
      ...
      // Clear any error
      context.__executingError = null;

      // console.log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! execute")
      // console.log(finalCommand)
      // console.dir(Object.keys(context))

      // Execute all commands
      yield self.executor.executeAsync(finalCommand, context);
      ...
      let finalResult = context.__result;
      ...
      if (context.__result instanceof Cursor && cmd !== 'it' /* && self.context.__result.cursor.s.state === 0 */) {
        ...
        // Read the first 20 entries or until empty
        while (true) {
          let doc = yield context.__result.next();
          // No more entries
          if (!doc) break;
          documents.push(doc);
          // Check if we reached the cutoff
        ...
        finalResult = finalResult.namespace;
      } else if (typeof finalResult === 'function') {
        finalResult = `${context.__namespace}.${finalResult.name}`;
  ...
  start() {
    ...
    readline.Interface.prototype.setPrompt = function(prompt) {
      if(self.context.db) {
        ...
        const dbName = self.context.db && self.context.db.name
          ? self.context.db.name
          : N/A;
        ...
        if (prompt && !prompt.match(/^\.\./)) {
          if (ismaster.hosts) {
            ...
            return $setPrompt.call(this, `mongodb [${palette.string.bold(dbName)}]:${serverType}> `, `mongodb [${dbName}]> `.length);
          }

          return $setPrompt.call(this, `mongodb [${palette.string.bold(dbName)}]> `, `mongodb [${dbName}]> `.length);
        }
      }
