...
function renderResponse(method, target, args, response) {
  ...
  let render = function() {
    return util.inspect(Object.assign(result, renderMapping[method](target, args, response)));
  };

  response.toString = render;
  response.inspect = render;
  return response;
}
    ...
    get(target, key, receiver) {
      ...
      if (typeof key === 'string') {
        if (typeof target[key] === 'undefined') {
          ...
          return new CollectionProxy(target.s.db.collection(collectionName));
        }
