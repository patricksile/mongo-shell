...
const { isAsyncFunction } = require('./utilities');
...
assert.contains = function(o, arr, msg) {
  ...
  if (!wasIn) {
    doassert(JSON.stringify(o) + ' was not in ' + JSON.stringify(arr) + ' : ' + msg);
  }
};
...
assert.commandWorked = function(res, msg) {
  ...
  doassert(`command failed: ${JSON.stringify(res)} : ${msg}`, res);
};
...
assert.commandFailed = function(res, msg) {
  ...
  doassert(`command worked when it should have failed: ${JSON.stringify(res)} : ${msg}`);
};
...
assert.commandFailedWithCode = function(res, code, msg) {
  ...
  assert.eq(res.code,
            code,
            'Expected failure code did not match actual in command result: ' + JSON.stringify(res) +
                ' : ' + msg);
  return res;
};
...
assert.isnull = function(what, msg) {
  ...
  doassert('supposed to be null (' + (msg || '') + ') was: ' + JSON.stringify(what));
};
