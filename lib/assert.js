...
assert.commandWorked = function(_res, msg) {
  if (assert._debug && msg) {
    console.log(`in assert for: ${msg}`);
  }

  function _commandWorked(res) {
  ...
  if (isPromise(_res)) {
    return _res.then(r => _commandWorked(r));
  }

  return _commandWorked(_res);
};
...
assert.commandFailedWithCode = function(_res, code, msg) {
  if (assert._debug && msg) {
    console.log(`in assert for: ${msg}`);
  }

  function _commandFailedWithCode(res) {
    assert(!res.ok,
  ...
  if (isPromise(_res)) {
    return _res
      .then(r => _commandFailedWithCode(r))
      .catch(e => { /* expected */ });
  }

  return _commandFailedWithCode(_res);
};
