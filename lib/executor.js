...
const ASYNC_METHODS = [
  ...
  'save', 'stats', 'update', 'updateMany', 'updateOne', 'validate',
  ...
  let output = falafel(src, function(node) {
    ...
    if (node.type === 'CallExpression') {
      ...
      if (ASYNC_METHODS.indexOf(node.callee.property.name) !== -1 && !isIIFE(node)) {
        ...
        // NOTE: special case for legacy `drop` support
        if (node.callee && node.callee.property && node.callee.property.name === 'drop') {
          node.update(`try { ${node.source()}; } catch(err) { console.warn('WARN: ' + err.message); }`);
        }
      } else {
