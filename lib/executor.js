...
//       in for non-assignment wrapping. No time to investigate atm.
function applyAsyncWrapper(node, options) {
  options = options || { assignment: false };
  if (options.assignment) {
    node.prepend('(() => { async function _wrap() { return global.');
    node.append('; } return _wrap(); })()');
  } else {
  ...
  let output = falafel(src, function(node) {
      ...
      if (node.right.awaited) {
        ...
        if (options.repl) {
          if (node.right.type !== 'FunctionExpression' &&
            ...
            // NOTE: special case to replace `preprocess` step in repl. This approach naively
            //       assigns the variable to the global context
            if (isProgramExpression(node)) {
              applyAsyncWrapper(node, { assignment: true });
    ...
    if (node.type === 'CallExpression') {
      ...
      if (ASYNC_METHODS.indexOf(node.callee.property.name) !== -1 && !isIIFE(node)) {
        ...
        if (options.repl) {
          if (node.awaited && isProgramExpression(node) && node.parent.type !== 'AssignmentExpression') {
            let programNode = findParent(node, [ 'Program' ]);
            if (!programNode.asyncWrapped) {
              applyAsyncWrapper(programNode);
