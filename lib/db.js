...
const getErrorWithCode = require('./helpers')._getErrorWithCode;
...
function hashPassword(username, password) {
  ...
  return crypto.createHash('md5').update(username + ':mongo:' + password).digest('hex');
}

function commandUnsupported(res) {
  return (res.errmsg.startsWith('no such cmd') || res.errmsg.startsWith('no such command') ||
...
class Db {
  ...
  static proxy(name, client, context = {}) {
    return new Proxy(new Db(name, client, context), {
      get: function(target, _name) {
        ...
        return new Collection(target.name, _name, target);
      }
    });
  }
  ...
  getSiblingDB(database) {
    return new Db(database, this.client, this.context);
  }
