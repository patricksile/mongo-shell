  ...
  autocomplete(context, hints) {
    ...
    if (namespace) {
      ...
      if (plugin) {
        ...
        log();
        log(palette.string.green(`${helpResult.description[descriptionKey].trim()}`));
        log(palette.string.green(`Example: ${this._generateExample(helpResult, line)}`));
        log();
      }
    }
  }
...
`);
      ...
      if (parts[0] === 'help' && parts.length == 2) {
        ...
        for (const _plugin of this._state.plugins) {
          if (_plugin.namespace() === parts[1]) {
            ...
            log(palette.string.bold(`Plugin ${_plugin.namespace()} Help\n=============`));
            // Map the entries
            ...
            // Render the entries
            log(columnify(entries, { showHeaders: false, config: { 0: { minWidth: 25 }, 1: {maxWidth: 80} } }));
          }
        }
      } else {
  ...
  _generateExample(doc, line = '') {
    ...
    let example = examples.length > 0 ? examples[0] : `${doc.ctx.name}(${params.join(', ')})`;
