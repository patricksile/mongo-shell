...
// Temporary plugin repo
const pluginsRepoJSONUri = `https://christkv.github.io/mongo-shell/plugins.json`;
...
class Plugins {
  static async cmd(code, state, log) {
    ...
    if (parts.length === 1) {
      return "available commands are list, search, install, remove";
      ...
      switch(parts[1]) {
        ...
        case 'search': {
          return await search(parts, state, log);
        }
    ...
    if (parts.length === 1) {
      return log(columnify(
        [
          ...
          [palette.string.green('help plugin search [term]'), 'Plugin search level help'],
      ...
      return log(`${palette.string.green('help plugin search [term]')}\nSearch plugin reposity, ommiting the search term will return all available plugins.`);
...
function search(parts, state, log) {
  return new Promise((resolve, reject) => {
    https.get(pluginsRepoJSONUri, (res) => {
      // The data returned
      let data = '';
      let search = parts.length === 3 ? parts[2] : undefined;
      // Our fetch was not succesful
      if (res.statusCode != 200) {
        return reject(new Error(`https fetch of ${pluginsRepoJSONUri} failed with code ${statusCode} and message ${res.statusMessage}`))
      }
      ...
      res.on('end', () => {
        try {
          ...
          // For all the entries generate a list
          for (const key of keys) {
            if(search) {
              if(key.toLowerCase().indexOf(search.toLowerCase()) != -1 
                || object[key].package.toLowerCase().indexOf(search.toLowerCase()) != -1 
                || object[key].description.toLowerCase().indexOf(search.toLowerCase()) != -1) {
                  descriptions.push([palette.string.green(key), palette.string.yellow(object[key].package), object[key].description]);            
              }
          ...
          // Render the columns
          resolve(columnify(descriptions, { showHeaders: false, config: { 
            0: { minWidth: 10 }, 
            1: {minWidth: 10}, 
            2: {maxWidth: 80} 
          } }));
        } catch(err) {
          reject(new Error(`Failed to parse json from ${pluginsRepoJSONUri}, errored with ${err}`));
        }
      })
    }).on('error', (e) => {
      reject(err);
  ...
  try {
    ...
    for (var _module of plugins) {
      try {
        ...
        pluginInstances.push(new plugin(state.client, {state: state}));
      } catch(err) {
